...
RUN <<-"EOD" bash
	...
	echo "deb [arch=amd64,arm64,armhf signed-by=/usr/share/keyrings/packages.microsoft.gpg] https://packages.microsoft.com/ubuntu/${OS_BASE}/prod ${OS_CODENAME} main" | tee /etc/apt/sources.list.d/microsoft-prod.list;
	#Set PHP Packages
	...
	#Install Packages
 echo $PKGS | tr ',' ' '
	ocie --dhparams "-size ${DH_PARAM_SIZE}" --pkg "-add $(echo $PKGS| tr ',' ' ')" --keys "-subject ${CERT_SUBJECT}";
